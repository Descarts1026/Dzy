...
class BackboneWithFPN(nn.Module):
    ...
    def __init__(self, backbone, return_layers, in_channels_list, out_channels, extra_blocks=None):
        ...
        self.body = IntermediateLayerGetter(backbone, return_layers=return_layers) #转成一个dict
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    if isinstance(norm_layer, FrozenBatchNorm2d): # 判断是否使用了FrozenBatchNorm2d
    ...
    # 如果要训练所有层结构的话 conv1后还有一个bn1 也要训练
    if trainable_layers == 5:
        layers_to_train.append("bn1")

    # freeze layers
    for name, parameter in resnet_backbone.named_parameters():
        # 只训练 layers_to_train列表中的层结构
